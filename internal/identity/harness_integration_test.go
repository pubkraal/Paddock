...
// startIdentityDB brings up a real Postgres with the two-role bootstrap applied,
// runs the app migrations as the BYPASSRLS migrate role, and returns a pool
// connected as the non-superuser paddock_app (so RLS applies) alongside the
// migrate *sql.DB used to seed across tenants.
func startIdentityDB(t *testing.T) (*postgres.Pool, *sql.DB) {
	t.Helper()
...
// applyMigrations runs every *.up.sql in order as the migrate role. pgx parses
// dollar-quoted bodies and multi-statement files, so each file applies whole.
...
// seedOrg inserts an organization as the BYPASSRLS migrate role (cross-tenant
// fixtures cannot go through the RLS-bound app role).
...
// seedUser inserts a user as the migrate role.
func seedUser(t *testing.T, db *sql.DB, id, orgID, email, role, status string) {
	t.Helper()

	const q = `INSERT INTO users (id, org_id, email, role, status) VALUES ($1, $2, $3, $4, $5)`
	if _, err := db.ExecContext(context.Background(), q, id, orgID, email, role, status); err != nil {
