...
Policies use the `missing_ok` form of `current_setting` — `current_setting('app.current_org', true)::uuid` — so an unscoped query (no GUC set) evaluates the predicate against `NULL` and sees **zero rows**, rather than raising `unrecognized configuration parameter`. This makes "fail closed" a graceful empty result on reads and a rejected `WITH CHECK` on writes, which is what the integration tier asserts.
